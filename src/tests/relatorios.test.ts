 ...
 * Spec: /relatorios/vendas/periodo · /relatorios/clientes/* · /relatorios/produtos/* · /relatorios/stock/baixo
 */
...
const RESUMO = {
  data_inicio: '2026-01-01T00:00:00Z', data_fim: '2026-12-31T23:59:59Z',
  total_vendas: 42, total_receita: 158000.5, total_pendente: 12000, total_sem_iva: 120000, total_iva: 28800,
  total_descontos: 5200, lucro_bruto: 45000, ticket_medio: 3761.92, produtos_mais_vendidos: [PRODUTO_VENDIDO],
}
const CLIENTE_FIEL = {
...
describe('relatoriosService.vendasPeriodo — GET /relatorios/vendas/periodo', () => {
  it('chama sem query params quando datas omitidas (traz hoje)', async () => {
    const spy = mockFetch(RESUMO)
    await relatoriosService.vendasPeriodo()
    expect(spy.mock.calls[0][0]).toBe(`${BASE}/relatorios/vendas/periodo`)
  })

  it('chama com data_inicio e data_fim como query params quando fornecidos', async () => {
    const spy = mockFetch(RESUMO)
  ...
  it('retorna RelatorioVendasPeriodo com total_pendente e produtos_mais_vendidos', async () => {
    mockFetch(RESUMO)
    ...
    expect(result.total_pendente).toBe(12000)
    expect(result.produtos_mais_vendidos).toHaveLength(1)
    expect(result.produtos_mais_vendidos[0].produto_nome).toBe('Arroz Agulha 5kg')
  })
})
