...
import type { DividaFornecedorResponse } from '@/types'
...
export default function DividaFornecedorDetalhePage() {
  const { id } = useParams<{ id: string }>()
  const navigate = useNavigate()
  const { t } = useTranslation()

  const [divida, setDivida] = useState<DividaFornecedorResponse | null>(null)
  ...
  useEffect(() => {
    if (!id) return
    setLoading(true)
    fornecedoresService.dividas.buscar(id)
      .then((d) => {
        setDivida(d)
        return fornecedoresService.buscar(d.fornecedor_id).catch(() => null)
      })
      .then((f) => setTelefone(f?.telefone ?? null))
  ...
  function handleBaixarPdf() {
    ...
    exportTablePdf({
      ...
      columns,
      rows: buildRows(),
      filename: `divida-fornecedor-${divida.id.slice(0, 8)}`,
      qrUrl: `${window.location.origin}/fornecedores/dividas/${divida.id}`,
    })
  }
  ...
  async function handlePartilhar() {
    ...
    try {
      ...
      const file = new File([blob], `divida-fornecedor-${divida.id.slice(0, 8)}.pdf`, { type: 'application/pdf' })
      const mensagem = [
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          {loading ? (
            ...
            <h1 className="text-2xl font-bold tracking-tight">{divida?.produto_nome ?? t('suppliers.colProduct')}</h1>
          )}
